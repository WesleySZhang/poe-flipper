...
type SortKey = "now" | "predicted" | "predictedPct" | "confidence";

async function fetchFlipSuggestions(currentDay: number, durationDays: number): Promise<FlipSuggestion[]> {
  const res = await fetch(`/api/flip-suggestions?currentDay=${currentDay}&durationDays=${durationDays}`);
  if (!res.ok) return [];
  return res.json();
}
...
export function CurrentLeagueTesterPanel() {
  ...
  const [sort, setSort] = useState<SortState<SortKey>>({ key: "predictedPct", direction: "desc" });
  const [priceUnit, setPriceUnit] = useState<PriceUnit>("chaos");
  const [isPending, startTransition] = useTransition();

  useEffect(() => {
    startTransition(async () => {
      setSuggestions(await fetchFlipSuggestions(currentDay, durationDays));
      setPage(0);
    });
    ...
    <Card>
      ...
      <CardContent className="flex flex-col gap-4">
        <p className="text-xs text-muted-foreground">
          &quot;Now&quot; is always today&apos;s real live price - only which historical (Current day &rarr; Current
          day + Days ahead) pattern gets applied to it changes. Actual future and Actual % are always &quot;—&quot;
          here: the league is still ongoing, so there&apos;s no record yet of what actually happens after today.
        </p>
        <CategoryFilter
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <SortableHeader label="Predicted %" sortKey="predictedPct" sort={sort} onSort={handleSort} />
